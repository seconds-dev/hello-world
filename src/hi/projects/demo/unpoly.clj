...
(html/defsnippet <project-list>
  ...
  [:#projects-list-main]
  [req]
  [:a#new-project-link] (html/set-attr :href
                                       "/demo/unpoly/projects/new"
                                       :up-size "large"
                                       :up-layer "new"
...
(html/defsnippet <project-comp>
  ...
  [:form] (html/set-attr :action (if (or (= "new" (-> req :path-params :id))
                                         (nil? (-> req :path-params :id)))
                                   (url req :demo.unpoly/projects {:id "new"} :? {:action "create"})
                                   (url req :demo.unpoly/project {:id (:id project)}))
                         :method "post"
                         :up-submit "")

  [:#new-company-link]
  (let [attrs (cond-> {:href (url req :demo.unpoly/companies)
                       :up-size "large"
                       :up-layer "new"}
                (or (= "new" (-> req :path-params :id))
                    (nil? (-> req :path-params :id)))
                (assoc :action (url req :demo.unpoly/products {:id "new"} :? {:action "create"})
                       :up-accept-location "/demo/unpoly/companies/$id")
                (not= "new" (-> req :path-params :id))
...
(html/defsnippet <company-comp>
  ...
  [:form] (html/set-attr :action (if (= "new" (-> req :path-params :id))
   ...
   req
    ...
    (let [project (-> req :params)]
      (cond
        ...
        {:body (apply str (<<main-page>> req (html/content (<project-comp> {:req req :project {:company "yoyoyo"}}))))}
        
        (= "666" (-> project :price))
        (let [req (merge req {:flash {:project-name-error "true"}})]
          {:body (apply str (<<main-page>> req (html/content (<project-comp> {:req req :project project}))))
         :status 422})
    ...
    {:request-method :get}
    (let [main-template (<<main-page>> req (html/content (<project-list> {:req req})))]
      {:body (apply str main-template)})

    :else (rr/not-found "Unsupported.")))

(defn project>> [req]
  (let [main-template (<<main-page>> req (html/content (<project-comp> {:req req})))]
    {:body (apply str main-template)}))
   ...
   req
    ...
    (if (= "error" (-> req :params :name))
      (let [company (merge (-> req :params) {:id 3})
            req (merge req {:flash {:project-name-error "true"}})
            main-template (<<main-page>> req (html/content (<company-comp> {:req req :company company})))]
        {:body (apply str main-template)
         :status 422})
      (rr/redirect (url req :demo.unpoly/company {:id 3})))

    {:request-method :get}
    (let [main-template (<<main-page>> req (html/content (<company-comp> {:req req})))]
      {:body (apply str main-template)})
